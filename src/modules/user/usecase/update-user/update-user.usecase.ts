...
export default class UpdateUserUseCase implements UpdateUserUseCaseInterface {
  ...
  async execute(data: UpdateUserUseCaseInputDto): Promise<UpdateUserUseCaseOutputDto> {
    ...
    if (losesAdminPrivileges) {
      await this.transactionManager.execute(
        async (trx) => {
          ...
          if (activeAdmins <= 1) {
            throw new ForbiddenError('Cannot remove privileges from the last active admin');
          }
