...
describe('User', () => {
  describe('create', () => {
    ...
    it('throws EntityValidationError when name is too short', () => {
      expect(() => User.create({ ...validProps(), name: 'x' })).toThrow(EntityValidationError);
    });

    it('throws EntityValidationError when password is empty', () => {
      expect(() => User.create({ ...validProps(), password: '' })).toThrow(EntityValidationError);
    });

    it('throws EntityValidationError when role is invalid', () => {
      expect(() => User.create({ ...validProps(), role: 'SUPREME' as UserRole })).toThrow(
        EntityValidationError,
      );
    });
  });
  ...
  describe('updateUser', () => {
    ...
    it('throws EntityValidationError when email is invalid', () => {
      const user = User.create(validProps());
      expect(() => user.updateUser({ email: 'bad-email' })).toThrow(EntityValidationError);
    });
  });
