...
const makeSut = () => {
  ...
  const validateSessionUseCase = {
    execute: jest.fn().mockResolvedValue({ userId: user.id, role: UserRole.ADMIN }),
  };
...
describe('UserFacade', () => {
  ...
  it.each([
    ['validateSession', 'validateSessionUseCase', { userId: 'id-1', issuedAt: 1 }],
  ...
  ] as const)('%s delegates to its use case', async (method, useCaseKey, input) => {
    const sut = makeSut();
