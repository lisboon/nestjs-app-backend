...
const makeSut = ({ user = makeUser(UserRole.EDITOR), activeAdmins = 2 } = {}) => {
  ...
  const useCase = new DeleteUserUseCase(transactionManager as any, userGateway as any);
...
describe('DeleteUserUseCase', () => {
  ...
  it('throws ForbiddenError when deleting the last active admin', async () => {
    ...
    await expect(useCase.execute({ id: admin.id })).rejects.toBeInstanceOf(ForbiddenError);
