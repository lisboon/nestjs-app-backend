...
const makeSut = ({ user = makeUser(), activeAdmins = 2 } = {}) => {
  ...
  const useCase = new UpdateUserUseCase(transactionManager as any, userGateway as any);
...
describe('UpdateUserUseCase', () => {
  ...
  it('throws ForbiddenError when demoting the last active admin', async () => {
    ...
    await expect(useCase.execute({ id: admin.id, role: UserRole.VIEWER })).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  ...
  it('throws ForbiddenError when deactivating the last active admin', async () => {
    ...
    await expect(useCase.execute({ id: admin.id, active: false })).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  });
