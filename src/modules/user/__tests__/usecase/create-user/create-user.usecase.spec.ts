...
const makeSut = ({ existingUserByEmail = null } = {}) => {
  ...
  const useCase = new CreateUserUseCase(userGateway as any, passwordHashService as any);
...
describe('CreateUserUseCase', () => {
  ...
  it('throws EntityValidationError when email is already taken', async () => {
    ...
    await expect(useCase.execute(validInput())).rejects.toBeInstanceOf(EntityValidationError);
  ...
  it('propagates entity validation when domain rules fail', async () => {
    ...
    await expect(useCase.execute({ ...validInput(), name: 'x' })).rejects.toBeInstanceOf(
